...
python -m fastchat.serve.controller --host 0.0.0.0 --port 10000 &
...
python -m fastchat.serve.gradio_web_server --controller http://localhost:10000 --model-list-mode reload --port 7860 & 
...
python -m fastchat.serve.model_worker --host 0.0.0.0 --controller http://localhost:10000 --model-path lmsys/vicuna-7b-v1.5 --device cpu & 
